...
class QuestionListView(ListView):
    ...
    template_name = 'poll/list.html' # 응답할 template파일경로를 지정.
    
    # 조회결과를 template에게 전달하는 dictionary(context_data, context_object)
...
class VoteView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            # 없는 question_id로 조회한 경우.=> 사용자가 없은 id의 설문을 요청
...
# DetailView: 특정 모델의 primary key값을 path parameter로 받아서 조회한 결과를 template에 전달하면서 호출.
